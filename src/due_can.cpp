...
uint32_t CANRaw::set_baudrate(uint32_t ul_baudrate)
{
  ...
  m_pCan->CAN_BR =
    CAN_BR_PHASE2(p_bit_time->uc_phase2 - 1) |
...
{
  if (mbox >= getNumMailBoxes() || txRings[mbox] != 0)
    return;
...
void CANRaw::initializeBuffers()
{
  ...
  if (tx_frame_buff == 0)
    tx_frame_buff = new CAN_FRAME[sizeTxBuffer];
  if (rx_frame_buff == 0)
    rx_frame_buff = new CAN_FRAME[sizeRxBuffer];
...
uint32_t CANRaw::init(uint32_t ul_baudrate)
{
  ...
  for (int i = 0; i < getNumMailBoxes() + 1; i++)
    cbCANFrame[i] = 0;
...
int CANRaw::setNumTXBoxes(int txboxes)
{
  ...
  if (txboxes > getNumMailBoxes())
    txboxes = getNumMailBoxes();
  if (txboxes < 0)
    txboxes = 0;
  numTXBoxes = txboxes;
...
uint16_t CANRaw::ringBufferCount(ringbuffer_t &ring)
{
  ...
  if (ring.tail == ring.head)
    return 0; //there's nothing in the buffer if they match
...
{
  uint32_t ul_reg;
  if (uc_index > CANMB_NUMBER - 1)
    uc_index = CANMB_NUMBER - 1;
...
bool CANRaw::sendFrame(CAN_FRAME &txFrame)
{
  ...
  { // If there is nothing buffered, find free mailbox
...
void CANRaw::interruptHandler()
{
  ...
  if (ul_status & CAN_SR_SLEEP)
  { //controller in sleep mode
  }
  if (ul_status & CAN_SR_WAKEUP)
  { //controller woke up
  }
  if (ul_status & CAN_SR_TOVF)
  { //timer overflow
  }
  if (ul_status & CAN_SR_TSTP)
  { //timestamp - start or end of frame
  }
  if (ul_status & CAN_SR_CERR)
  { //CRC error in mailbox
  }
  if (ul_status & CAN_SR_SERR)
  { //stuffing error in mailbox
    numBusErrors++;
  }
...
void CANRaw::mailbox_int_handler(uint8_t mb, uint32_t /* ul_status */)
{
  ...
  if (mb >= getNumMailBoxes())
    mb = getNumMailBoxes() - 1;
  if (m_pCan->CAN_MB[mb].CAN_MSR & CAN_MSR_MRDY)
  { //mailbox signals it is ready
    switch (((m_pCan->CAN_MB[mb].CAN_MMR >> 24) & 7))
    { //what sort of mailbox is it?
    case 1: //receive
    ...
    case 3: //transmit
      ...
      if (removeFromRingBuffer(*pRing, tempFrame))
